...
# Login
TOKEN=$(curl -Fpassword=$ASPACE_PASSWORD $ASPACE_URL/users/$ASPACE_USERNAME/login | jq '.session' | cut -d \" -f 1)
...
function getRepository {
    ...
    curl -H "X-ArchivesSpace-Session: $TOKEN" $ASPACE_URL/repositories |  jq -r ".[].uri" | cut -d / -f 3 > data/repository-ids.txt
    ...
    cat data/repository-ids.txt | while read REPO_ID; do
        if [ "$REPO_ID" != "" ]; then
            curl -H "X-ArchivesSpace-Session: $TOKEN" $ASPACE_URL/repositories/$REPO_ID > data/repositories/$REPO_ID.json
        fi
    done
